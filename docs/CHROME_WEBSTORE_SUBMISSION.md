# Chrome Web Store — Submission Notes (E-Vault Password Manager)
...
> E-Vault Password Manager is a password manager. Its single purpose is to securely autofill, capture,
...
> already-logged-in E-Vault Password Manager web-app tab so the extension can connect without re-entering
> credentials.
...
> purpose, and transmits data only to the user's configured E-Vault Password Manager backend over HTTPS.
...
- **Authentication information** — account email + session token (used to sign in to the
  user's E-Vault Password Manager backend).
...
> E-Vault Password Manager requires a user account on a E-Vault Password Manager backend to function. Please use this
...
>
> Steps to test:
>   1. Click the E-Vault Password Manager toolbar icon to open the popup.
>   2. Sign in with the demo email + password above, then unlock with the same password.
>   3. Visit any website with a login form. E-Vault Password Manager will offer to autofill saved logins,
...
2. **Your own server** — copy `privacy-policy.html` to the E-Vault Password Manager host, e.g.
